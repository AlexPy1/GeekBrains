...
let all_but = document.querySelectorAll('button');
all_but.forEach(element => {
...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = ;
    let prod = clickedButtonEvent.target.parentNode 

    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: prod, // здесь элемент с классом .product
        img: prod.querySelector('img'), // здесь картинка внутри .product
        productName: prod.querySelector('.productName'), // здесь .productName, который внутри .product
        button: prod.querySelector('button') // здесь button, который внутри .product
    };
